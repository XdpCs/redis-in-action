...
func TestClient_ArticleVote(t *testing.T) {
	...
	oneArticleID := c.PostArticle("XdpCs", "Xdp's girlfriend", "https://github.com/XdpCs")
	twoArticleID := c.PostArticle("XdpCs", "Xdp's girlfriend", "https://github.com/XdpCs")
	c.ArticleVote("XdpCs1", common.ArticleKey+oneArticleID)
	c.ArticleVote("XdpCs2", common.ArticleKey+oneArticleID)
	all := client.HGetAll(ctx, common.ArticleKey+oneArticleID).Val()
	if all["votes"] != "3" {
		t.Fatalf("actual:[%v] expected:[%v]", all["votes"], "3")
	}
	c.ArticleVote("XdpCs1", common.ArticleKey+twoArticleID)
	c.ArticleVote("XdpCs2", common.ArticleKey+twoArticleID)
	all = c.client.HGetAll(ctx, common.ArticleKey+twoArticleID).Val()
	if all["votes"] != "3" {
		t.Fatalf("actual:[%v] expected:[%v]", all["votes"], "3")
	}
}
