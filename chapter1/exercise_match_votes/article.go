...
func (c *Client) ArticleVote(user, article, voteType string) {
	...
	if c.client.SAdd(ctx, common.VotedKey+voteType+articleID, user).Val() == 1 {
		switch voteType {
...
func (c *Client) MatchVotes(user, article string) {
	cutoff := float64(time.Now().Unix()) - common.OneWeekInSecond.Seconds()
	ctx := context.Background()
	if c.client.ZScore(ctx, common.TimeKey, article).Val() < cutoff {
		return
	...
	articleID := strings.Split(article, ":")[1]
	voteType := common.AffirmativeVotes
	for _, r := range c.client.SMIsMember(ctx, common.VotedKey+common.OpposingVotes+articleID, user).Val() {
		if r {
			voteType = common.OpposingVotes
			break
		}
	}

	if c.client.SMove(ctx, common.VotedKey+voteType+articleID, common.VotedKey+common.ReverseVoteType(voteType)+articleID, user).Val() {
		switch voteType {
		case common.AffirmativeVotes:
			c.client.ZIncrBy(ctx, common.ScoreKey, -2*common.VoteScore, article)
			c.client.HIncrBy(ctx, article, common.AffirmativeVotes, -1)
			c.client.HIncrBy(ctx, article, common.OpposingVotes, 1)
		case common.OpposingVotes:
			c.client.ZIncrBy(ctx, common.ScoreKey, 2*common.VoteScore, article)
